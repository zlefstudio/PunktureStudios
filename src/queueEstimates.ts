...
export function calculateQueueWaitTimes(
  ...
  let rollingWaitMinutes = tickets.filter(t => t.status === 'called')
    .reduce((sum, t) => sum + (durations?.get(t.id) ?? getTicketDuration(byTicket.get(t.id) ?? [])), 0);
  let inProgressEstimate: InProgressEstimate | undefined = undefined;

  if (inProgressTicket) {
    const ipItems = byTicket.get(inProgressTicket.id) ?? [];
    const totalDuration = (durations?.get(inProgressTicket.id) ?? getTicketDuration(ipItems));
  ...
  for (const ticket of waitingTickets) {
    ...
    const ownDuration = (durations?.get(ticket.id) ?? getTicketDuration(ticketSpecificItems));
