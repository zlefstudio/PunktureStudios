...
**Current booking environment (2026-09-14 — LIVE mode, QR Ph):** Worker `punkture-booking` runs `PAYMONGO_LIVE=true` against a **verified but unregistered (Individual)** PayMongo account, so `PAYMENT_METHODS=qrph` only. PayMongo restricts direct Cards/GCash/Maya/GrabPay/ShopeePay/online-banking channels to registered business types (DTI/SEC); an Individual account can accept **QR Ph**, which any participating bank or e-wallet app can scan — including GCash. QR Ph costs **1.34%** (PHP 1.34 on the PHP 100.00 deposit) versus GCash 2.23% and cards 3.125% + PHP 13.39. The live secret key and the live webhook signing secret are Worker secrets — never in the repo or in a `VITE_` variable. `GET /availability` returns HTTP 200, and a preflight checkout renders a QR Ph method instead of the old "No payment methods are available" dead end. Customers can now hand a slot back immediately with `POST /bookings/:id/release` (the Worker verifies with the provider first, so an already-paid slot is confirmed instead of freed). A real PHP 100.00 live payment, its signed webhook delivery and both inboxes are still **unverified** — do not describe live acceptance as tested until that smoke payment is recorded. Two pre-live test-mode bookings remain in the D1 ledger (PHP 200 gross, annotated in `audit`) and must be excluded from real revenue. Set `BOOKING_LAUNCH_READY=false` and redeploy to pause new checkout creation.
