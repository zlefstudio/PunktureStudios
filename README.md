...
**Retired preset upgrade:** `upgradeLegacySchedule()` in the staff page, public page and Worker recognizes only the exact old flat list `13:00,14:30,16:00,17:30,19:00` with all seven weekdays enabled and no `bookingDaySlots`. That preset resolves to the current studio grid immediately when the new code runs, and the staff's next save persists it. Existing custom grids/day selections and closures are preserved. **Apply studio hours** remains available to reset a custom weekly schedule while keeping date exceptions.
