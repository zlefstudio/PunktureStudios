...
**Current booking environment (2026-09-14 — LIVE mode):** Worker `punkture-booking` runs `PAYMONGO_LIVE=true` against a verified live PayMongo account with `PAYMENT_METHODS=gcash` (GCash only; the studio absorbs provider fees). The live secret key (`sk_live_…`) and the new live webhook signing secret are Worker secrets — never in the repo or in a `VITE_` variable. `GET /availability` returns HTTP 200, so every readiness gate passes (legacy import, mode flag, live-key prefix, non-empty webhook secret, mailer configuration). A real PHP 100.00 live payment, its signed webhook delivery and both inboxes are still **unverified** — do not describe live acceptance as tested until that smoke payment is recorded. Two pre-live test-mode bookings remain in the D1 ledger (PHP 200 gross, annotated in `audit`) and must be excluded from real revenue. Set `BOOKING_LAUNCH_READY=false` and redeploy to pause new checkout creation.
...
**Live PayMongo account switch (2026-09-14):** The studio replaced the original PayMongo test account with a verified live account. `backend/wrangler.jsonc` now carries `PAYMONGO_LIVE="true"`, `PAYMENT_METHODS="gcash"` and `BOOKING_LAUNCH_READY="true"` (Worker version `d9d8b6c0-6246-4af6-9603-1d61f25c2637`). Because `ready()` requires the secret key prefix to match `PAYMONGO_LIVE`, a key/mode mismatch fails every booking endpoint closed with HTTP 503 — including `/availability` — so the old `sk_test_…` key cannot be left bound to a live-mode Worker, and the flip must land together. `PAYMENT_METHODS` is GCash-only because the studio absorbs fees: GCash 2.23% (PHP 2.23 on the PHP 100.00 deposit) versus cards 3.125% + PHP 13.39 (about PHP 16.50, 16.5%). Add a channel only after it is activated on the account: an unactivated `payment_method_types` value makes checkout creation fail while the booking row still holds the slot for 15 minutes with `checkout_creation_uncertain`. Live webhook events sign with `li` (test uses `te`), and the signing secret is unique per endpoint, so a new endpoint requires a new secret. Pre-live `expired` rows had their old-account `session_id`/`checkout_url` cleared so the per-minute cron reconciler stops requesting sessions that no longer exist on the new account; the two test-mode `confirmed` rows are retained for the ledger and annotated in `audit` as `prelive_test_payment_old_paymongo_account`. A D1 export backup was taken before the writes. Real-payment, webhook-delivery and inbox acceptance remain pending.
