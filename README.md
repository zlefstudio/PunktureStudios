...
2. **Step 2: Schedule & Slots**:
   ...
   - Respects studio operating days (`bookingDays`), the per-weekday slot grid (`bookingDaySlots`, legacy `bookingSlots` still honoured), advance notice (`bookingNoticeDays`, default 1 day), and blackout dates (`blockedDates`). Days with no slot (Sunday by default) are not selectable, and the slot header prints the day's first–last slot.
...
| `public/public` | Public | Staff only | Studio profile, pop-up announcements, and booking rules (`bookingDays`, `bookingDaySlots` per-weekday grid, legacy `bookingSlots`, `blockedDates`). |
...
**Waiver modal is now full screen and responsive:** the consent gate (`booking/WaiverReviewModal.tsx`) fills a mobile screen (square corners, no outer padding) and becomes a tall centred panel on desktop (`sm:max-w-2xl`, `lg:max-w-3xl`, height capped at `100dvh-2rem`). A pinned header keeps the title/language toggle/close visible, the waiver copy scrolls inside (`flex-1 min-h-0 overflow-y-auto overscroll-contain`), and the consent checkbox plus **Agree & Continue to PHP 100.00 Payment** sit in a pinned footer above the mobile safe area (`env(safe-area-inset-bottom)`). **Follow-up fix (same day):** the modal is now rendered through `createPortal(..., document.body)` because PublicShell's content wrapper ends a `pk-fade-in` animation on `transform: translateY(0)`, which becomes the containing block for `position:fixed` descendants — rendered inline the panel was fixed to the content column, so the pay button landed below the fold and the customer had to scroll the page to hunt for it. The page is also pinned (overflow/position saved and restored) while the modal is open, and `.waiver-modal` carries a `100vh` fallback before `100dvh` for WebViews that predate the dynamic viewport unit. The policy line reads “the studio waiver above” since consent moved below the copy. Covered by a structural test: the dialog is portalled to `<body>`, the panel is full height, the copy scrolls inside, and the pay button/consent sit outside the scroll area.
...
**Settings page (`PublicSettingsView.tsx` → Schedule panel):** the flat "Available Time Slots" chip list is replaced by a studio-hours card plus a **per-weekday editor**. The card prints `Mon–Fri 9:00 AM – 12:00 PM, 1:00 PM – 8:00 PM · Sat 1:00 PM – 5:00 PM · Sun closed · 45-minute appointments`, tells the staff whether the **saved** schedule already matches it, and offers **Apply studio hours** (one click sets `bookingDays` to Mon–Sat, reseeds `bookingDaySlots` from the built-in hours and refreshes the flat list) with a reminder to press *Save all changes*. Each weekday then shows **its effective saved list** (the legacy flat list until the preset is applied) as removable chips, labelled by block (`9:00 AM – 11:15 AM, 1:00 PM – 7:45 PM · 14 slots`) plus its own time input and `Add` button; switching a day off removes it from the map, switching it on keeps that day's current slots (or seeds the studio hours when there are none). **One studio action is still required:** until the staff applies + saves this preset, the live `public/public` document keeps whatever schedule it already had (the previous 13:00/14:30/16:00/17:30/19:00 list on all seven days), because the page treats saved settings as authoritative.
...
**Verification (2026-09-15):** `npm run lint` clean, `npm run build` OK, `npm test` **100/100 passing** (new `tests/schedule.test.mjs` for the grid, worker parity and settings validation; component tests for the label/CTA/back-button changes, the 45-minute grid on weekdays + Saturday, the deposit bullets and the settings preset; `npm run test:rules` **14/14 passing** with the new per-weekday map cases). **Nothing was deployed and no live Firestore document was written by this task** — the grid reaches customers only when the staff applies the preset in **Settings → Schedule → Apply studio hours → Save all changes**. Publish in this order to avoid a booking dead end: (1) apply + save the settings (the deployed Worker reads the flat union and already accepts every new slot), (2) deploy the Worker so validation uses the per-weekday map, (3) publish the public pages. Existing confirmed bookings were deliberately left untouched (they were the studio's own test bookings).
