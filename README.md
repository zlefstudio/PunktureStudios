...
1. **Step 1: Placement Selection**:
   ...
   - Per-placement initial-jewelry rules on the booking modal (hotspot `jewelryPrices`): Navel = included Surgical Steel only; Floating Navel = +200 tier only; Rook = 150 titanium only (no rhinestone). The staff cashier is unaffected.
...
- **Spark optimization (2026-09-24):** `remoteRows.ts` retains server-confirmed Firestore listeners across sync cycles; cache-only/pending snapshots are not merged. The 30-second safety sync no longer scans whole collections on every tick. Counter writes are change-only and heartbeat writes are throttled to at most once per 25 seconds. Restore starts fresh server listeners; sign-out/offline/error resets them. See the release notes and quota runbook below.
...
- **Honest preload:** `home/preloadFirstMedia.ts` waits only for the decoded first image/poster from `mediaItems[0]`, with an **850ms hard cap**. The playing video owns its video request; the intro no longer downloads a disposable video copy. Failed/aborted loads cannot strand the curtain; no pretend progress counter or whole-gallery readiness dependency is used. Existing orbit posters may load normally, but only that one media asset gates the sequence. Gallery `<Media>` components do not mount until handoff and keep their native lazy loading afterward.
...
**Historical torso layout (superseded 2026-09-25):** The first shared torso removed the separate chest view. The current image, anchors and preview layout are documented under “Booking placement and cart polish” below. The nipple catalog entry remains a single piercing; the left/right/both selector belongs to ear piercings only.

**Historical chest marker correction:** The earlier `(292, 328)` coordinate belonged to `body-reference.png`; it must not be reused with the new 527×520 torso. See the current source-pixel coordinates below.
...
**Body placements on one graphic:** Navel, Floating Navel and Nipple (single) share one torso. The 2026-09-25 update replaces the previous vertically separated navel targets with lateral labelled callouts pointing to a shared upper-rim anchor. Only ear piercings offer a left/right/both selector.
...
## Spark quota conservation (2026-09-24)

The launch screenshot shows 4.47 GB of **seven-day** Hosting downloads and 36K current Firestore reads. These are not a verified month-to-date total or a guarantee of future traffic. The audit, official quota references, capacity estimates and release/rollback procedure are in [docs/spark-budget.md](docs/spark-budget.md).

**Staff sync:** `src/remoteRows.ts` owns persistent server-confirmed listeners for `tickets`, `items`, `publicQueue` and the single allowed `public/public` document. `sync.ts` takes per-cycle working copies rather than refetching full collections every 30 seconds. Cache-only or pending-write snapshots block merging until confirmed; a 15-second wait timeout reports a retryable sync error. Sign-out, offline transitions and sync errors release listeners. Restore deliberately obtains fresh server snapshots, retains durable restore intent and all deletion tombstones. Other-device updates still arrive live and the 30-second safety cycle remains. Counter writes occur on value changes/session reset; heartbeat writes are limited to one per 25 seconds, with the normal 30-second safety tick and existing 90-second stale threshold. Local locks, queue ordering, history, port 5174 and private staff-only hosting boundary remain intact.

**Public listeners/polling:** `LiveQueuePage.tsx` subscribes to heartbeat only while a nonempty cloud queue exists. `visibleSubscription.ts` disconnects its queue/heartbeat listeners after 30 seconds hidden, retaining connections across brief tab switches and reconnecting on return. It clears stale heartbeat status before resuming. `AppointmentPage.tsx` retains visible 10-second availability polling, skips hidden requests, prevents overlapping requests, and refreshes after return, discarding responses from a previous visibility period. Checkout, atomic holds, payments and the Worker remain unchanged.

**Media delivery:** `scripts/media-assets.ts` converts all 28 home-manifest files (12 reels, 12 posters, four gallery graphics; **37.54 MB total**) into content-hashed production assets. Original compressed bytes and editable source filenames remain intact. `/assets/**` now has a one-year immutable browser cache; original unversioned media has a one-hour cache policy. HTML keeps its default revalidation behavior. The intro decodes only its first poster, avoiding an extra video request; reels, autoplay, orbit, viewer and reduced-motion behavior remain. Generated `dist/media-manifest.json` inventories only public media.

**Free media host:** `media/wrangler.jsonc` defines the independent `punkture-media` static-assets host, with no booking Worker code, D1 or secrets. It is published at `https://punkture-media.zlef-dev.workers.dev` (version `c283a60e-fdcc-4f85-b299-2cae4cb50801`). `npm run media:prepare` stages only manifest files and cache headers in ignored `dist-media/`; it retains previously staged hashes for older open pages. Vite and GitHub Actions default to this verified host; a nonempty repository variable `VITE_MEDIA_BASE_URL` overrides the CI origin. An explicitly empty local `VITE_MEDIA_BASE_URL` builds self-contained Firebase media; for a CI rollback also change the workflow's media URL to an explicit empty string. `npm run media:verify` checks every file's content type, exact size and immutable caching before frontend release. It accepts a valid HTTP 206 range response or HTTP 200 full-file response with the exact content length; Cloudflare currently returns the latter. Manual deploy and GitHub Actions perform this check when configured. Publish media changes before the frontend and preserve older media hashes between deployments. No paid upgrade or database/rules migration is required.

**Released (2026-09-25):** Cloudflare CLI login is complete and Firebase Hosting deployment to `punkture-studios` succeeded (Hosting only). All 28 media files are published and passed live file-size/content-type/cache verification. The live `https://punkture-studios.web.app/home` played its reel from `https://punkture-media.zlef-dev.workers.dev`, readyState 4 and no media error. All seven public routes returned HTTP 200; the deployed media manifest has the correct external origin and all 28 assets. The deployed appointment bundle matches the tested build and retains `https://punkture-booking.zlef-dev.workers.dev`. No live settings, Firestore rules, booking Worker or billing plan were changed. Reload the local cashier at `localhost:5174` to activate staff sync savings. Source edits remain in the local working tree; no Git commit/push was performed.

**Validation:** 137/137 tests passed, `npm run lint` and `npm run build` passed, including the production configuration with the existing booking API and external media origin. Tests cover retained listener updates, cache/pending-snapshot rejection, listener failure recovery, heartbeat throttling, restore/tombstone safety, background subscription/polling behavior, and all 28 emitted media files retaining their exact source bytes with external hashed URLs. Browser verification confirmed reel playback in the viewer with background reels paused, and the live queue's empty state and guide links. All 28 remote media checks passed. Post-release quota savings still need observation over comparable traffic days; Firestore rules were unchanged.

## Admin navigation and jewelry labels (2026-09-25)

The staff Add Piercing panel uses plain SERVICES and JEWELRY tabs and removes the ring emoji from jewelry choices. Queue, History and Settings navigation now uses matching Lucide SVG icons with visible text labels and decorative icons hidden from assistive technology. The shared upgrade catalog and compact staff labels use **Surgical Steel** (0) and **Rhinestone Jewelry** (50); prices and titanium choices are unchanged. Shared booking selectors inherit these catalog labels on their next public release. Existing saved orders are not rewritten: formatting normalizes legacy material labels, copied breakdowns use the new wording, and staff upgrade selection matches the stable price tier so older orders remain correctly selected. Waiver/legal copy is outside this admin presentation change. Reload the local cashier at localhost:5174 to see the update; the staff app remains local-only.

## Booking placement and cart polish (2026-09-25)

**Current body reference:** `public/media/anatomy/body-torso-v2.jpg` is the supplied 527×520 JPEG copied without altering its bytes (63,725 bytes). `referenceGeometry.ts` scales image and source-pixel coordinates together by 400/527. Nipple (single) follows the user's red-dot guide on the image's right breast at source pixel (405, 208), replacing (152, 214); the annotated screenshot is a positioning guide only and is not shipped as an asset; the reference has no nipple texture, so the caption explicitly says it is illustrative and for one piercing on either side. Navel and Floating Navel use separate labelled lateral tap targets, with leader lines and jewelry offsets to the same (266, 452) upper-rim anchor. Floating jewelry has a flat lower end; it is no longer depicted as a lower navel hole. These coordinates supersede the older PNG coordinates above. Exact suitability remains an in-person assessment.

**Placement details:** `PiercingMap` supports optional callouts and a focused preview viewBox. Body detail previews show the chest or navel at a useful size; all modal previews use a full-width image with a compact title and landmark caption. Removing the old short-screen wrapper height fixes the image overlapping the anatomy warning. Scrollable content has an explicit minimum height of zero, while totals and Add to cart / Save changes stay outside the scroll area. Narrow screens stack footer actions; short landscape screens use a compact cart footer. Jewelry and ear-side controls expose pressed state. Existing catalog names, prices and tier restrictions remain intact, including the shared Surgical Steel and Rhinestone Jewelry labels.

**Cart flow:** The persistent cart control shows item quantity (both ears count twice) and total. Add, edit and remove actions announce confirmation; the cart shows service/jewelry pricing, both-ear quantity, Edit and Remove controls, Continue choosing, and the existing step-aware scheduling action. Only mapped catalog placements offer Edit; services remain removable. `cartSnapshot.upsertCartItem` replaces edits by stable id so changing side cannot duplicate a row. A both-ear choice replaces separate selections of that placement with the chosen jewelry, explained in the selector. Adding a new single-ear line to an existing pair preserves the opposite ear's jewelry. The cart hides during detail/waiver dialogs, payment and paused booking. `useDialogFocus` traps keyboard focus, restores an opener or the cart control, and supports safe transitions between the two dialogs. Session draft persistence, booking notes, scheduling, consent, deposit and server operations retain their existing contracts; cart changes make no backend writes.

Validation: 44 targeted component/cart/map tests passed, including edit identity, side overlap, price totals, keyboard trapping, dialog scroll-lock transitions and no booking writes during cart editing. Browser checks used the local appointment page at 320×568, 390×844, 768×1024, 1280×800 and 844×390 landscape. The new image and detail/cart layouts were inspected visually; small-screen controls remain reachable and measured viewports have no horizontal overflow. Final validation: **141/141 tests passed**, `npm run lint` passed and `npm run build` passed. Editing also preserves existing item notes. Direct map-label selection and cart-to-schedule navigation passed in the browser, with no console errors or test booking/payment submission. Changes are local; this UI follow-up has not been deployed.
